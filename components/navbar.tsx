...
export function Navbar({ bannerHeight = 0 }: { bannerHeight?: number }) {
  return (
    <nav
      className="fixed inset-x-0 z-50 border-b-[3px] border-[#111] bg-[#ff5f1f] text-[#111]"
    ...
    >
      <div className="mx-auto flex h-16 max-w-[1440px] items-center justify-between px-6 lg:px-9">
        <Link href="/" className="flex items-center gap-3 text-[#111]">
          ...
          <span className="hidden font-mono text-[10px] font-bold uppercase tracking-[0.22em] text-[#111] lg:inline">
            Beag Labs
          </span>
        </Link>
        ...
        <div className="flex items-center gap-4 sm:gap-6 lg:gap-8">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className="hidden border-b-2 border-transparent text-[11px] font-extrabold uppercase tracking-[0.1em] text-[#111] transition-all hover:border-[#111] sm:inline"
            >
              {link.label}
            </Link>
          ))}
